...
let replacementsAsCharacters: [(Character, Character)] = [(":", "꞉"),
                                                          ("/", ":"),
                                                          ("\0", "␀")]
...
let benchmarks = {
    ...
    for (sampleCore, label) in [
        ...
        for lengthModifier in sampleCore.isEmpty ? [1] : [1, 10, 100, 1_000, 10_000, 100_000, 1_000_000] {
            ...
            Benchmark("[\(label) ⨉\(lengthModifier.formatted())] N-pass via replacingOccurrences") { benchmark in
            ...
            Benchmark("[\(label) ⨉\(lengthModifier.formatted())] N-pass via replace") { benchmark in
                for _ in benchmark.scaledIterations {
                    var result = sample
            ...
            Benchmark("[\(label) ⨉\(lengthModifier.formatted())] N-pass via replacing") { benchmark in
                for _ in benchmark.scaledIterations {
            ...
            Benchmark("[\(label) ⨉\(lengthModifier.formatted())] Single pass via character enumeration & concatenation") { benchmark in
                for _ in benchmark.scaledIterations {
                    var result = ""
            ...
            Benchmark("[\(label) ⨉\(lengthModifier.formatted())] Single pass via character enumeration & concatenation, with naive space reservation") { benchmark in
                for _ in benchmark.scaledIterations {
                    var result = ""
            ...
            Benchmark("[\(label) ⨉\(lengthModifier.formatted())] Two pass via character enumeration & concatenation, with accurate space reservation") { benchmark in
                for _ in benchmark.scaledIterations {
            ...
            Benchmark("[\(label) ⨉\(lengthModifier.formatted())] Two pass via character enumeration & concatenation, with accurate space reservation & String(unsafeUninitializedCapacity:initializingUTF8With:)") { benchmark in
                for _ in benchmark.scaledIterations {
                    var outputByteCount = 0
                    var willReplace = false
            ...
            Benchmark("[\(label) ⨉\(lengthModifier.formatted())] Single pass via map & join") { benchmark in
                for _ in benchmark.scaledIterations {
